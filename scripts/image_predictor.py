...
def single_box_reuse_example(processor, image, width, height):
    ...
    inference_state = processor._add_box_prompt(box=norm_box_cxcywh, label=True, state=inference_state)
    ...
    modifications = {
        ...
        "grayscale": lambda img: img.convert("L").convert(
            "RGB"
        ),  # Convert to L then back to RGB to maintain 3 channels
    }
...
def cross_image_reuse_example(processor, image1, image2, width1, height1):
    ...
    # Add the prompt and extract the encoded features
    inference_state1 = processor._add_box_prompt(box=norm_box_cxcywh, label=True, state=inference_state1)
